...
class WindowShortcuts {
  ...
  addShortcut(shortcut: ShortcutExt) {
    ...
    if (this.shortcuts[id]) {
      console.warn(`Shortcut [${ getId(shortcut) }] is already registered for window: [${ this.window.id }]; Skip.`);
  ...
  removeShortcut(shortcut: Shortcut) {
    ...
    if (!this.shortcuts[id]) {
      console.warn(`Shortcut [${ getId(shortcut) }] is not registered for window: [${ this.window.id }]; Skip.`);
